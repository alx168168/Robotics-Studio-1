...
public:
    ScanToImageNode() : Node("scan_to_image_node"), angle_difference_(0.0), relative_orientaion_(0.0) {
        ...
        // Load the edge map from the home directory
        std::string home_dir = getenv("HOME");
        std::string file_path = home_dir + "/edge_map.png";
        m_edge_map = cv::imread(file_path, cv::IMREAD_GRAYSCALE);
        if (m_edge_map.empty()) {
            RCLCPP_ERROR(this->get_logger(), "Failed to load edge map from %s", file_path.c_str());
        }
    }
